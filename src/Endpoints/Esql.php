...
class Esql extends AbstractEndpoint
{
	 ...
	 *     allow_partial_dsl_filter?: bool, // If `true`, unsupported clauses in the request `filter` are silently dropped and the translatable subset is applied; a `Warning` header lists the dropped constructs. If `false` (default), any unsupported construct fails the query with a 400.
	...
	public function asyncQuery(?array $params = null)
	{
		...
		$url = $this->addQueryString($url, $params, ['format','delimiter','drop_null_columns','allow_partial_results','allow_partial_dsl_filter','pretty','human','error_trace','source','filter_path']);
		$headers = [
