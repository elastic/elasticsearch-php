...
use Iprice\Elasticsearch\ConnectionPool\SniffingConnectionPool;
use Mockery as m;
...
class SniffingConnectionPoolTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testAddOneHostAndTriggerSniff()
    {
        ...
        $connections = array($mockConnection);
        $mockNewConnection = m::mock('\Iprice\Elasticsearch\Connections\Connection')
                             ->shouldReceive('isAlive')->andReturn(true)->getMock();
    ...
    public function testAddOneHostSniffTwo()
    {
        ...
        $newConnections[] = m::mock('\Iprice\Elasticsearch\Connections\Connection')
                             ->shouldReceive('isAlive')->andReturn(true)->getMock();
    ...
    public function testTen_TimeoutNine_SniffTenth_AddTwoAlive()
    {
        ...
        foreach (range(1, 10) as $index) {
            $mockConnection = m::mock('\Iprice\Elasticsearch\Connections\Connection')
                              ->shouldReceive('ping')->andReturn(false)->getMock()
                              ->shouldReceive('isAlive')->andReturn(true)->getMock()
                              ->shouldReceive('sniff')->andThrow('Iprice\Elasticsearch\Common\Exceptions\Curl\OperationTimeoutException')->getMock();
