...
namespace Iprice\Elasticsearch\Tests\ConnectionPool;
...
class StaticConnectionPoolTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $mockConnection = m::mock('\Iprice\Elasticsearch\Connections\Connection')
                          ->shouldReceive('ping')
                          ->andReturn(true)
                          ->getMock()
        ...
        $selector = m::mock('\Iprice\Elasticsearch\ConnectionPool\Selectors\RoundRobinSelector')
                    ->shouldReceive('select')
                    ->andReturn($connections[0])
                    ->getMock();

        $connectionFactory = m::mock('\Iprice\Elasticsearch\Connections\ConnectionFactory');

        $randomizeHosts = false;
        $connectionPool = new \Iprice\Elasticsearch\ConnectionPool\StaticConnectionPool($connections, $selector, $connectionFactory, $randomizeHosts);
    ...
    /**
     * @expectedException \Iprice\Elasticsearch\Common\Exceptions\NoNodesAvailableException
     */
    ...
    public function testAllExceptLastHostFailPingRevivesInSkip()
    {
        ...
        $goodConnection = m::mock('\Iprice\Elasticsearch\Connections\Connection')
    ...
    {
        $clientBuilder = \Iprice\Elasticsearch\ClientBuilder::create();
        $clientBuilder->setHosts(['localhost:1']);
        $client = $clientBuilder
            ->setRetries(0)
            ->setConnectionPool('\Iprice\Elasticsearch\ConnectionPool\StaticConnectionPool', [])
            ->build();

        try {
            $client->search([]);
            $this->fail("Should have thrown NoNodesAvailableException");
        } catch (\Iprice\Elasticsearch\Common\Exceptions\NoNodesAvailableException $e) {
            // All good
        } catch (\Exception $e) {
            throw $e;
        }
